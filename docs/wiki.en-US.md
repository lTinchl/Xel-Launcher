...
This guide is for first-time Xel Launcher users. It covers installation, first-time setup, game launch, server switching, account management, sign-in, updates, and common issues
...
- Your system is Windows 10 22H2 (build 19045) or later
- Microsoft Edge WebView2 Runtime is installed. The launcher checks for it at startup and cannot open the main window without it
- You have downloaded the installer or portable package from the project Releases page
- Shared runtimes require the game to be on an NTFS partition and the launcher to have permission to create runtime directories in the game's parent directory. The runtime and game are on the same partition; the launcher itself does not need to be on that partition
...
- Traditional server switching and manual replacement overwrite some server-specific files in the target game directory. The launcher also falls back to traditional switching if a shared runtime is unavailable. Always confirm that the selected directory is the correct game root before running it
...
### Preload and Integrity Verification

- When a preload is available, a `Preload` button appears beside the launch button. Use it to download update resources in advance
- If game files are missing or damaged, select `Verify Game Integrity` from the lower-right more menu and follow the prompts to check and repair files
...
4. The launcher performs the following steps as needed:
   ...
   - Prepares a shared runtime or switches server-specific files using the traditional method
...
When multiple server entries share the same game directory, the launcher prepares a runtime for the target server or replaces server files in the original game directory, depending on the `Use hard links for server switching` setting
...
4. When launching different entries, the launcher automatically prepares the corresponding server's runtime. You do not need to create or initialize runtime directories manually

### Shared Runtimes Using Hard Links

With `Use hard links for server switching` enabled for compatible server entries sharing the same path, the launcher identifies the base channel currently installed in the original game directory. It launches that channel directly from the original directory. For other channels, it creates a separate runtime directory in the game's parent directory and launches the game from there

- The configured game directory provides the source files. Common resources that match in both channels' file manifests and meet the sharing rules receive hard links in the runtime. Both paths share the same data on disk
- Channel-specific files and files that need separate storage are copied or downloaded. Files that cannot be reused locally may require downloading, so enabling hard links does not eliminate network access or downloads
- Initial setup fetches manifests, checks local resources, and prepares files, so it may take longer than subsequent launches. The base channel must be updated to the version in the remote manifest. If a shared runtime cannot be created, the launcher displays a message and falls back to traditional server switching
- The configured game path remains the original game directory; do not change it to the runtime path. Disabling hard links returns the launch flow to the original directory, replacing channel files there as needed

Hard links have no primary or secondary direction: changing shared file contents through either path affects the other. Configuration and other files that require separate storage are not shared this way

### Manual Server File Replacement

Exit the game first, then copy the server-specific files for the intended channel into the game installation directory yourself. This overwrites the related files, so confirm that both the target directory and channel are correct

### Update Server Switching Resources

Select `Update Server Files` from the lower-right more menu to open the differential file update window. It checks and updates resources for the corresponding channels and lets you enable automatic updates at launcher startup per channel. This updates the files needed for server switching; full client updates still use `Update Game` on the game page
...
Expand the tools in the lower-left corner of the game page and click the Skyland icon to open the sign-in window, then select the platform you need. It currently includes:
...
Use `+` in the tool sidebar to add a custom link for the current game entry. Enter a name and URL, and optionally select a local icon. Right-click an added custom tool to delete it
...
- `Save launcher background image`: archive fetched launcher images by game and date in the `img` folder under the launcher directory
- `Use hard links for server switching`: prefer runtimes that share common resources when multiple compatible server entries use the same game directory. Disabling it uses traditional server switching. See [Shared Runtimes Using Hard Links](#shared-runtimes-using-hard-links)

The settings window also includes:

- `Logs`: view launcher runtime logs
- `Software Update`: check for new versions, select a download source, and download the update installer or portable package
...
Shared runtime directories are stored in the configured game directory's parent directory:

```text
<game directory's parent>\.xel-linked-runtime\<game ID>\<path ID>\<channel>
```

During creation, the launcher uses a temporary `.staging` directory under the same path ID directory, then renames it to the target channel directory on success. Keep the game path setting pointed at the original installation

Other useful locations:

| Content | Location |
| --- | --- |
| Runtime logs | `logs\app.log` under the launcher directory |
| Server switching resources | `load` under the launcher directory |
| Image cache | `%LocalAppData%\XelLauncher\GameCovers` |
| Image archive (when saving is enabled) | `img` under the launcher directory |
...
Initial runtime creation takes time to fetch manifests, check resources, and copy or download independent files. Falling back to traditional switching when hard links are unavailable can also take longer. Recommended checks:

- Confirm that compatible server entries point to the same game directory and enable `Use hard links for server switching` in settings
- Confirm that the game is on NTFS, its directory is not a symbolic link or junction, and the launcher has permission to create runtime directories
- Update the base client, check your network connection, and inspect launcher logs for the reason for any fallback
...
Xel Launcher is an unofficial tool and is not affiliated with or partnered with Hypergryph or its affiliated organizations. Copyrights to game images, names, and data belong to their respective rights holders
