...
namespace XelLauncher.Forms
{
    public class AccountManagerForm : UserControl
    {
        ...
        private void Table_SortRows(object sender, AntdUI.IntEventArgs e)
        {
            ...
            if (_iconName == "Endfield") cfg.EndfieldAccountOrder = newOrder;
            else if (_iconName == "GlobalEndfield") cfg.GlobalEndfieldAccountOrder = newOrder;
        ...
        private void Table_CellButtonClick(object sender, AntdUI.TableButtonEventArgs e)
        {
            ...
            switch (e.Btn.Id)
            {
                ...
                case "setDefault":
                    ...
                    else if (_iconName == "GlobalEndfield") cfg.GlobalEndfieldDefaultAccount = row.id;
                ...
                case "delete":
                    ...
                    else if (_iconName == "GlobalEndfield")
                    {
                        cfg2.GlobalEndfieldAccounts.Remove(row.id);
                        cfg2.GlobalEndfieldAccountOrder.Remove(row.id);
                        if (cfg2.GlobalEndfieldDefaultAccount == row.id) cfg2.GlobalEndfieldDefaultAccount = "";
                    }
                    else
                    {
        ...
        private void ShowAddDialog()
        {
            ...
            {
                cfg.GlobalEndfieldAccounts[id] = name;
                cfg.GlobalEndfieldAccountOrder.Add(id);
                if (string.IsNullOrEmpty(cfg.GlobalEndfieldDefaultAccount)) cfg.GlobalEndfieldDefaultAccount = id;
            }
            else
            {
        ...
        private void ShowRenameDialog(string id, string currentName)
        {
            ...
            var cfg = ConfigHelper.Load();
            if (_iconName == "Endfield") cfg.EndfieldAccounts[id] = name;
            else if (_iconName == "GlobalEndfield") cfg.GlobalEndfieldAccounts[id] = name;
